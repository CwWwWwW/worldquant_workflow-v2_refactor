...
class OutcomeTrainer:
    ...
    def _load_samples(self, limit: int = 10000) -> list[dict[str, Any]]:
        ...
        try:
            ...
            rows = conn.execute("SELECT * FROM simulator_training_samples ORDER BY created_at DESC LIMIT ?", (limit,)).fetchall()
            out = []
            for r in rows:
                d = dict(r); features = json_loads_safe(d.get("features_json"), {})
                for k in ("fitness", "sharpe", "turnover"):
                    if d.get(k) is not None: features.setdefault(k, d.get(k))
                success = d.get("backtest_success") if d.get("backtest_success") is not None else d.get("quality_passed")
                label = {"reward": d.get("reward"), "success": success}
                if d.get("turnover") is not None:
                    label["high_turnover_risk"] = 1 if float(d.get("turnover") or 0.0) > float(getattr(self.config, "simulator_high_turnover_threshold", 0.7) or 0.7) else 0
                out.append({"features": features, "label": label, "alpha_id": d.get("alpha_id"), "raw": d})
            return out
        except Exception:
            return []
        finally:
    ...
    def train(self) -> dict[str, Any]:
        try:
            if not bool(getattr(self.config, "enable_simulator_model_training", False)):
            ...
            samples = self._load_samples(); min_samples = int(getattr(self.config, "simulator_learning_min_samples", 200) or 200)
            labeled = [s for s in samples if (s.get("label") or {}).get("reward") is not None]
            ...
            x = np.asarray([[row.get(n, 0.0) for n in names] for row in flat], dtype=float); y = np.asarray([float((s.get("label") or {}).get("reward") or 0.0) for s in labeled], dtype=float)
            ...
            metrics: dict[str, Any] = {"sample_count": len(labeled), "train_sample_count": len(labeled)}
            ...
            report = build_evaluation_report("simulator", metrics, self.config)
            active = False; version = ""
            if self.model_registry:
                meta = self.model_registry.save_model_version("simulator", {"models": models}, SimpleFeatureSchema(feature_names=names), train_sample_count=len(labeled), evaluation=report, model_type="simulator_random_forest", raw_payload={"trainer": "OutcomeTrainer"})
                if meta and bool(meta.get("ok", True)):
                    version = str(meta.get("model_version") or "")
                    if validation_gate("simulator", metrics, self.config).get("passed"):
                        active = bool(self.model_registry.activate_model("simulator", version, reason="validation_passed"))
