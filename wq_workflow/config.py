...
STRATEGY_CHALLENGER_MIN_SAMPLES = 100
STRATEGY_LIMITED_ACTIVE_MIN_SAMPLES = 500
STRATEGY_CHALLENGER_MIN_CONFIDENCE = "medium"
STRATEGY_LIMITED_ACTIVE_MIN_CONFIDENCE = "high"
STRATEGY_BLOCK_HIGH_SC_RISK = True
...
STRATEGY_BUDGET_TOTAL_HINT = 200
STRATEGY_BUDGET_FAIL_OPEN = True
STRATEGY_BUDGET_LEGACY_MIN_RATIO = 0.40
STRATEGY_BUDGET_EXPLORATION_MIN_RATIO = 0.05
STRATEGY_BUDGET_SHADOW_MAX_RATIO = 0.02
STRATEGY_BUDGET_CHALLENGER_MAX_RATIO = 0.20
STRATEGY_BUDGET_LIMITED_ACTIVE_MAX_RATIO = 0.35
STRATEGY_BUDGET_HIGH_RISK_MAX_RATIO = 0.05
STRATEGY_BUDGET_HIGH_SC_MAX_RATIO = 0.05
STRATEGY_BUDGET_INSUFFICIENT_EVIDENCE_MAX_RATIO = 0.02
STRATEGY_BUDGET_NON_CHAMPION_MAX_RATIO = 0.35
STRATEGY_BUDGET_NORMALIZATION_TOLERANCE = 0.001
STRATEGY_BUDGET_AUTO_APPLY = False
STRATEGY_BUDGET_ALLOCATOR_AUTO_APPLY = False
ENABLE_OBSERVABILITY_METRICS = True
OBSERVABILITY_METRICS_STATUS_PATH = "runtime/status/observability_metrics.json"
OBSERVABILITY_MODE = "metrics_only"
OBSERVABILITY_AUTO_COLLECT = False
OBSERVABILITY_FAIL_OPEN = True
OBSERVABILITY_STATUS_MAX_AGE_SECONDS = 86400
OBSERVABILITY_RECENT_WINDOW = 100
OBSERVABILITY_METRICS_DEFAULT_LIMIT = 1000
OBSERVABILITY_COLLECT_WORKFLOW = True
OBSERVABILITY_COLLECT_ML = True
OBSERVABILITY_COLLECT_GOVERNANCE = True
OBSERVABILITY_COLLECT_EXPERIMENT = True
OBSERVABILITY_COLLECT_OFFLINE = True
OBSERVABILITY_COLLECT_STRATEGY = True
OBSERVABILITY_COLLECT_SYSTEM = True
ENABLE_OBSERVABILITY_ALERTS = False
ENABLE_OBSERVABILITY_DRIFT_DETECTION = False
ENABLE_OBSERVABILITY_DIAGNOSIS = False
OBSERVABILITY_ALERTS_STATUS_PATH = "runtime/status/observability_alerts.json"
OBSERVABILITY_DIAGNOSIS_STATUS_PATH = "runtime/status/health_diagnosis.json"
OBSERVABILITY_ALERT_MODE = "advisory"
OBSERVABILITY_DIAGNOSTICS_AUTO_RUN = False
OBSERVABILITY_DRIFT_WINDOW_SIZE = 20
OBSERVABILITY_DRIFT_BASELINE_WINDOW_SIZE = 100
OBSERVABILITY_FAILURE_SPIKE_THRESHOLD = 0.5
OBSERVABILITY_SUCCESS_DROP_THRESHOLD = 0.3
OBSERVABILITY_SC_RISK_THRESHOLD = 0.7
OBSERVABILITY_WARNING_COUNT_THRESHOLD = 5
OBSERVABILITY_DISK_LOW_MB_THRESHOLD = 1024
OBSERVABILITY_SOURCE_STALE_SECONDS = 86400
OBSERVABILITY_DIAGNOSIS_FAIL_OPEN = True
ENABLE_RUN_EXPLAINABILITY = False
RUN_EXPLAIN_REPORT_STATUS_PATH = "runtime/status/run_explain_report.json"
DAILY_OBSERVABILITY_REPORT_STATUS_PATH = "runtime/status/daily_observability_report.json"
STAGE7_SUMMARY_REPORT_STATUS_PATH = "runtime/status/stage7_summary_report.json"
OBSERVABILITY_EXPLAINABILITY_MODE = "explain_only"
OBSERVABILITY_EXPLAINABILITY_AUTO_RUN = False
OBSERVABILITY_EXPLAINABILITY_FAIL_OPEN = True
OBSERVABILITY_EXPLANATION_RECENT_LIMIT = 1000
OBSERVABILITY_EXPLANATION_INCLUDE_COUNTERFACTUAL = True
OBSERVABILITY_EXPLANATION_INCLUDE_BUDGET = True
OBSERVABILITY_EXPLANATION_INCLUDE_ALERTS = True
OBSERVABILITY_EXPLANATION_INCLUDE_DIAGNOSIS = True
OBSERVABILITY_EXPLANATION_AUTO_ACTION = False
ENABLE_LEGACY_ITERATION_OBSERVER = True
LEGACY_RUNTIME_STATE_PATH = "runtime/status/runtime_state.json"
LEGACY_RECENT_EVENTS_PATH = "runtime/status/recent_events.jsonl"
LEGACY_LEARNING_EVIDENCE_PATH = "runtime/status/legacy_learning_evidence.jsonl"
LEGACY_OBSERVER_FAIL_OPEN = True
LEGACY_OBSERVER_MAX_EVENT_PAYLOAD_CHARS = 1000
LEGACY_OBSERVER_MAX_MESSAGE_CHARS = 300
LEGACY_RECENT_EVENTS_MAX_BYTES = 5_242_880
LEGACY_LEARNING_EVIDENCE_MAX_BYTES = 10_485_760
LEGACY_OBSERVER_WRITE_RUNTIME_STATE = True
LEGACY_OBSERVER_WRITE_RECENT_EVENTS = True
LEGACY_OBSERVER_WRITE_LEARNING_EVIDENCE = True
LEGACY_OBSERVER_INCLUDE_ALPHA_EXPRESSION = False
LEGACY_OBSERVER_INCLUDE_TEMPLATE_BODY = False
LEGACY_OBSERVER_INCLUDE_TRACEBACK = False
OBSERVABILITY_ALERT_AUTO_EMIT = False
OBSERVABILITY_AUTO_REMEDIATION = False
...
def load_config() -> WorkflowConfig:
    ...
    config = WorkflowConfig(
        ...
        strategy_portfolio_status_path=str(raw.get("strategy_portfolio_status_path") or STRATEGY_PORTFOLIO_STATUS_PATH),
        strategy_portfolio_mode=str(raw.get("strategy_portfolio_mode") or STRATEGY_PORTFOLIO_MODE),
        strategy_portfolio_auto_refresh=_as_bool(raw.get("strategy_portfolio_auto_refresh"), STRATEGY_PORTFOLIO_AUTO_REFRESH),
        strategy_portfolio_fail_open=_as_bool(raw.get("strategy_portfolio_fail_open"), STRATEGY_PORTFOLIO_FAIL_OPEN),
        strategy_default_champion=str(raw.get("strategy_default_champion") or STRATEGY_DEFAULT_CHAMPION),
        strategy_challenger_min_samples=max(1, _as_int(raw.get("strategy_challenger_min_samples"), STRATEGY_CHALLENGER_MIN_SAMPLES)),
        strategy_limited_active_min_samples=max(1, _as_int(raw.get("strategy_limited_active_min_samples"), STRATEGY_LIMITED_ACTIVE_MIN_SAMPLES)),
        strategy_challenger_min_confidence=str(raw.get("strategy_challenger_min_confidence") or STRATEGY_CHALLENGER_MIN_CONFIDENCE),
        strategy_limited_active_min_confidence=str(raw.get("strategy_limited_active_min_confidence") or STRATEGY_LIMITED_ACTIVE_MIN_CONFIDENCE),
        strategy_block_high_sc_risk=_as_bool(raw.get("strategy_block_high_sc_risk"), STRATEGY_BLOCK_HIGH_SC_RISK),
        strategy_allow_auto_champion_promotion=_as_bool(raw.get("strategy_allow_auto_champion_promotion"), STRATEGY_ALLOW_AUTO_CHAMPION_PROMOTION),
        strategy_transition_auto_apply=_as_bool(raw.get("strategy_transition_auto_apply"), STRATEGY_TRANSITION_AUTO_APPLY),
        enable_strategy_budget_allocator=_as_bool(raw.get("enable_strategy_budget_allocator"), ENABLE_STRATEGY_BUDGET_ALLOCATOR),
        strategy_budget_status_path=str(raw.get("strategy_budget_status_path") or STRATEGY_BUDGET_STATUS_PATH),
        strategy_budget_mode=str(raw.get("strategy_budget_mode") or STRATEGY_BUDGET_MODE),
        strategy_budget_auto_refresh=_as_bool(raw.get("strategy_budget_auto_refresh"), STRATEGY_BUDGET_AUTO_REFRESH),
        strategy_budget_total_hint=max(0, _as_int(raw.get("strategy_budget_total_hint"), STRATEGY_BUDGET_TOTAL_HINT)),
        strategy_budget_fail_open=_as_bool(raw.get("strategy_budget_fail_open"), STRATEGY_BUDGET_FAIL_OPEN),
        strategy_budget_legacy_min_ratio=max(0.40, min(1.0, _as_float(raw.get("strategy_budget_legacy_min_ratio"), STRATEGY_BUDGET_LEGACY_MIN_RATIO))),
        strategy_budget_exploration_min_ratio=max(0.05, min(1.0, _as_float(raw.get("strategy_budget_exploration_min_ratio"), STRATEGY_BUDGET_EXPLORATION_MIN_RATIO))),
        strategy_budget_shadow_max_ratio=max(0.0, min(1.0, _as_float(raw.get("strategy_budget_shadow_max_ratio"), STRATEGY_BUDGET_SHADOW_MAX_RATIO))),
        strategy_budget_challenger_max_ratio=max(0.0, min(1.0, _as_float(raw.get("strategy_budget_challenger_max_ratio"), STRATEGY_BUDGET_CHALLENGER_MAX_RATIO))),
        strategy_budget_limited_active_max_ratio=max(0.0, min(1.0, _as_float(raw.get("strategy_budget_limited_active_max_ratio"), STRATEGY_BUDGET_LIMITED_ACTIVE_MAX_RATIO))),
        strategy_budget_high_risk_max_ratio=max(0.0, min(1.0, _as_float(raw.get("strategy_budget_high_risk_max_ratio"), STRATEGY_BUDGET_HIGH_RISK_MAX_RATIO))),
        strategy_budget_high_sc_max_ratio=max(0.0, min(1.0, _as_float(raw.get("strategy_budget_high_sc_max_ratio"), STRATEGY_BUDGET_HIGH_SC_MAX_RATIO))),
        strategy_budget_insufficient_evidence_max_ratio=max(0.0, min(1.0, _as_float(raw.get("strategy_budget_insufficient_evidence_max_ratio"), STRATEGY_BUDGET_INSUFFICIENT_EVIDENCE_MAX_RATIO))),
        strategy_budget_non_champion_max_ratio=max(0.0, min(1.0, _as_float(raw.get("strategy_budget_non_champion_max_ratio"), STRATEGY_BUDGET_NON_CHAMPION_MAX_RATIO))),
        strategy_budget_normalization_tolerance=max(0.0, _as_float(raw.get("strategy_budget_normalization_tolerance"), STRATEGY_BUDGET_NORMALIZATION_TOLERANCE)),
        strategy_budget_auto_apply=False if "strategy_budget_auto_apply" not in raw else False,
        strategy_budget_allocator_auto_apply=False if "strategy_budget_allocator_auto_apply" in raw else STRATEGY_BUDGET_ALLOCATOR_AUTO_APPLY,
        enable_observability_metrics=_as_bool(raw.get("enable_observability_metrics"), ENABLE_OBSERVABILITY_METRICS),
        observability_metrics_status_path=str(raw.get("observability_metrics_status_path") or OBSERVABILITY_METRICS_STATUS_PATH),
        observability_mode=str(raw.get("observability_mode") or OBSERVABILITY_MODE),
        observability_auto_collect=_as_bool(raw.get("observability_auto_collect"), OBSERVABILITY_AUTO_COLLECT),
        observability_fail_open=_as_bool(raw.get("observability_fail_open"), OBSERVABILITY_FAIL_OPEN),
        observability_status_max_age_seconds=max(0, _as_int(raw.get("observability_status_max_age_seconds"), OBSERVABILITY_STATUS_MAX_AGE_SECONDS)),
        observability_recent_window=max(1, _as_int(raw.get("observability_recent_window"), OBSERVABILITY_RECENT_WINDOW)),
        observability_metrics_default_limit=max(1, _as_int(raw.get("observability_metrics_default_limit"), OBSERVABILITY_METRICS_DEFAULT_LIMIT)),
        observability_collect_workflow=_as_bool(raw.get("observability_collect_workflow"), OBSERVABILITY_COLLECT_WORKFLOW),
        observability_collect_ml=_as_bool(raw.get("observability_collect_ml"), OBSERVABILITY_COLLECT_ML),
        observability_collect_governance=_as_bool(raw.get("observability_collect_governance"), OBSERVABILITY_COLLECT_GOVERNANCE),
        observability_collect_experiment=_as_bool(raw.get("observability_collect_experiment"), OBSERVABILITY_COLLECT_EXPERIMENT),
        observability_collect_offline=_as_bool(raw.get("observability_collect_offline"), OBSERVABILITY_COLLECT_OFFLINE),
        observability_collect_strategy=_as_bool(raw.get("observability_collect_strategy"), OBSERVABILITY_COLLECT_STRATEGY),
        observability_collect_system=_as_bool(raw.get("observability_collect_system"), OBSERVABILITY_COLLECT_SYSTEM),
        enable_observability_alerts=_as_bool(raw.get("enable_observability_alerts"), ENABLE_OBSERVABILITY_ALERTS),
        enable_observability_drift_detection=_as_bool(raw.get("enable_observability_drift_detection"), ENABLE_OBSERVABILITY_DRIFT_DETECTION),
        enable_observability_diagnosis=_as_bool(raw.get("enable_observability_diagnosis"), ENABLE_OBSERVABILITY_DIAGNOSIS),
        observability_alerts_status_path=str(raw.get("observability_alerts_status_path") or OBSERVABILITY_ALERTS_STATUS_PATH),
        observability_diagnosis_status_path=str(raw.get("observability_diagnosis_status_path") or OBSERVABILITY_DIAGNOSIS_STATUS_PATH),
        observability_alert_mode="advisory",
        observability_alert_auto_emit=False,
        observability_diagnostics_auto_run=_as_bool(raw.get("observability_diagnostics_auto_run"), OBSERVABILITY_DIAGNOSTICS_AUTO_RUN),
        observability_auto_remediation=False,
        observability_drift_window_size=max(1, _as_int(raw.get("observability_drift_window_size"), OBSERVABILITY_DRIFT_WINDOW_SIZE)),
        observability_drift_baseline_window_size=max(1, _as_int(raw.get("observability_drift_baseline_window_size"), OBSERVABILITY_DRIFT_BASELINE_WINDOW_SIZE)),
        observability_failure_spike_threshold=max(0.0, _as_float(raw.get("observability_failure_spike_threshold"), OBSERVABILITY_FAILURE_SPIKE_THRESHOLD)),
        observability_success_drop_threshold=max(0.0, _as_float(raw.get("observability_success_drop_threshold"), OBSERVABILITY_SUCCESS_DROP_THRESHOLD)),
        observability_sc_risk_threshold=max(0.0, _as_float(raw.get("observability_sc_risk_threshold"), OBSERVABILITY_SC_RISK_THRESHOLD)),
        observability_warning_count_threshold=max(1, _as_int(raw.get("observability_warning_count_threshold"), OBSERVABILITY_WARNING_COUNT_THRESHOLD)),
        observability_disk_low_mb_threshold=max(0, _as_int(raw.get("observability_disk_low_mb_threshold"), OBSERVABILITY_DISK_LOW_MB_THRESHOLD)),
        observability_source_stale_seconds=max(0, _as_int(raw.get("observability_source_stale_seconds"), OBSERVABILITY_SOURCE_STALE_SECONDS)),
        observability_diagnosis_fail_open=_as_bool(raw.get("observability_diagnosis_fail_open"), OBSERVABILITY_DIAGNOSIS_FAIL_OPEN),
        enable_run_explainability=_as_bool(raw.get("enable_run_explainability"), ENABLE_RUN_EXPLAINABILITY),
        run_explain_report_status_path=str(raw.get("run_explain_report_status_path") or RUN_EXPLAIN_REPORT_STATUS_PATH),
        daily_observability_report_status_path=str(raw.get("daily_observability_report_status_path") or DAILY_OBSERVABILITY_REPORT_STATUS_PATH),
        stage7_summary_report_status_path=str(raw.get("stage7_summary_report_status_path") or STAGE7_SUMMARY_REPORT_STATUS_PATH),
        observability_explainability_mode="explain_only",
        observability_explainability_auto_run=_as_bool(raw.get("observability_explainability_auto_run"), OBSERVABILITY_EXPLAINABILITY_AUTO_RUN),
        observability_explainability_fail_open=_as_bool(raw.get("observability_explainability_fail_open"), OBSERVABILITY_EXPLAINABILITY_FAIL_OPEN),
        observability_explanation_recent_limit=max(1, _as_int(raw.get("observability_explanation_recent_limit"), OBSERVABILITY_EXPLANATION_RECENT_LIMIT)),
        observability_explanation_include_counterfactual=_as_bool(raw.get("observability_explanation_include_counterfactual"), OBSERVABILITY_EXPLANATION_INCLUDE_COUNTERFACTUAL),
        observability_explanation_include_budget=_as_bool(raw.get("observability_explanation_include_budget"), OBSERVABILITY_EXPLANATION_INCLUDE_BUDGET),
        observability_explanation_include_alerts=_as_bool(raw.get("observability_explanation_include_alerts"), OBSERVABILITY_EXPLANATION_INCLUDE_ALERTS),
        observability_explanation_include_diagnosis=_as_bool(raw.get("observability_explanation_include_diagnosis"), OBSERVABILITY_EXPLANATION_INCLUDE_DIAGNOSIS),
        observability_explanation_auto_action=False,
